...
A guide to using the dev-workflow CLI and Claude Code plugin for managing tasks across multiple spaces.
...
dev-workflow space create harness-eng --description "Harness engineering"
```
...
```
  harness              Default workspace              3 tasks
  personal             Personal projects              1 task
  harness-eng          Harness engineering             0 tasks
```
...
The default space is `harness`. It is auto-created on first use -- no setup required:

```bash
# This just works, even on a fresh install:
dev-workflow task start "My first task" --prompt "Build something"
```
...
The active space applies to all commands in that invocation. You cannot operate on two spaces in a single command (except `task list --all-spaces`).
...
The plugin provides slash commands that orchestrate the CLI with Superpowers skills. This is the intended way to use dev-workflow inside Claude Code sessions.
...
### Scenario: personal project + org work in parallel
...
```bash
# Everything across all spaces
dev-workflow task list --all-spaces

# Output:
#   [harness]     auth-refactor    execution    Auth Middleware Rewrite
#   [harness]     csv-export       plan         CSV Export Feature
